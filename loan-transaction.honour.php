...
<body class="sidebar-noneoverflow">
        ...
        <header class="header navbar navbar-expand-sm">
            ...
            <ul class="navbar-item flex-row navbar-dropdown">
                ...
                <li class="nav-item dropdown message-dropdown">
                    <a onclick="location.href='mailto:contact@azurecreditunion.com'" href="javascript:void(0);"
                            ...
                            <table id="default-ordering" class="table table-hover" style="width:100%">
                                ...
                                if ($result->num_rows > 0) {
                                    ...
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        // Assuming $row['amount'] contains the loan amount
                                        $loanAmount = $row['amount'];
                                        $extraInterest = 0.16 * $loanAmount; // Calculate 16% extra interest
                                
                                        // Add the extra interest to the loan amount
                                        $totalAmount = $loanAmount + $extraInterest;

                                        // Output the loan amount with currency symbol and extra interest
                                        echo "<td>" . $acct_currency . $loanAmount . " (+" . $acct_currency . number_format($extraInterest, 2) . "  interest)</td>";

                                        echo "<td>" . (strlen($row['loan_remarks']) > 20 ? substr($row['loan_remarks'], 0, 20) . "..." : ($row['loan_remarks'] ? $row['loan_remarks'] : '-')) . "</td>";
                                        echo "<td>" . $status . "</td>";
                                        if($row['status'] == 1){
                                        echo "<td><button class='btn btn-primary repay-btn' data-loan-id='" . $row['loan_id'] . "'>Repay Loan</button></td>";
                                        }else{
                                            echo "<td>-</td>";
                                        }        
    ...
    <!-- Place this code inside your HTML page -->
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@10"></script>
    <script>
        $(document).ready(function () {
            // Event handler for clicking the "Repay Loan" button
            $('.repay-btn').click(function (e) {
                e.preventDefault(); // Prevent default link behavior

                // Get the loan ID from the data attribute
                var loanId = $(this).data('loan-id');

                // Send an AJAX request
                $.ajax({
                    url: 'reimburse.php', // URL to your reimburse.php file
                    type: 'POST',
                    data: { id: loanId },
                    success: function (response) {
                        ...
                        // Determine the type of SweetAlert based on response.status
                        var alertType = response.status == 'success' ? 'success' : 'error';

                        // Display SweetAlert with appropriate type
                        Swal.fire({
                            icon: alertType,
                            title: response.title, // Title from reimburse.php
                            text: response.message // Message from reimburse.php
                        });
