...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
  ...
  $sql = "SELECT * FROM wire_transfer WHERE user_id = (SELECT id FROM user_registration WHERE username = ?) ORDER BY id DESC";
  ...
  if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        if($row['status'] == 0){
          ...
          echo "<td>" . $row['acct_country'] . "</td>";
          echo "<td>" . $row['acct_swift'] . "</td>";
          echo "<td>" . $row['acct_routing'] . "</td>";
