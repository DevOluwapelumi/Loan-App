...
<head>
    ...
    <title>Withdawal Transaction - <?php echo $sitename?>  </title>
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
  ...
  $sql = "SELECT * FROM withdrawal WHERE user_id = (SELECT id FROM user_registration WHERE username = ?) ORDER BY withdrawal_id DESC";
  ...
  if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_assoc()) {
        if($row['status'] == 1){
            $status = '<span class="badge outline-badge-primary shadow-none col-md-12">Completed</span>';
          }elseif(($row['status'] == 0)){
            $status = '<span class="badge outline-badge-danger shadow-none col-md-12">Hold</span>';
        }else{
            $status = "Failed";
        }
          ...
          echo "<td>" . $fullname . "</td>";
          echo "<td>" . md5($row['withdrawal_id']) . "</td>";
